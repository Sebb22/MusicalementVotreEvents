      ...
      <table class="dashboard-table">
        ...
        <tbody>
          <?php foreach ($categories as $item): ?>
            <tr data-item='<?= htmlspecialchars(json_encode($item), ENT_QUOTES) ?>'>
              <td data-label="Nom"><?= htmlspecialchars($item->name) ?></td>
              <td data-label="Catégorie" class="category-name"><?= htmlspecialchars($item->location_name ?? '-') ?></td>
              <td data-label="Prix"><?= number_format($item->price, 2, ',', ' ') ?> €</td>
              <td data-label="Stock"><?= $item->stock ?></td>
              <td data-label="Disponibilité"><?= $item->availability ? 'Disponible' : 'Indisponible' ?></td>
