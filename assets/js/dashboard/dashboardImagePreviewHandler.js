export function initImagePreview({
    ...
    async function generateTransformedImage() {
      ...
      console.log('[Debug] Container :', containerRect.width, containerRect.height);
      console.log('[Debug] Image naturelle :', img.naturalWidth, img.naturalHeight);
      console.log('[Debug] Transform :', 'translateX=', translateX, 'translateY=', translateY, 'scale=', scale);
      console.log('[Debug] Portion source :', visibleX, visibleY, visibleW, visibleH);
  
      ctx.clearRect(0, 0, canvas.width, canvas.height);
      ctx.drawImage(img, visibleX, visibleY, visibleW, visibleH, 0, 0, canvas.width, canvas.height);
      ...
      console.log('[Debug] Image transformée dans hiddenInput :', hiddenInput.value?.substring(0,100) + '...');
    }
    ...
    return {
      ...
      setImage(src) {
        ...
        removeBtn.style.display = src ? 'block' : 'none';
        translateX = 0;
        translateY = 0;
        scale = 1;
        resizeInput.value = scale * 100;
